...
def compute_likelihoods_and_variance(input_dfs, likelihood_thresh=0.9):
    ...
    for part in bodypart_list:
        keys.extend(
            [keypoint_is[part + '_x'], keypoint_is[part + '_y'], keypoint_is[part + '_likelihood']]
        )
...
def plot_heatmaps(likelihoods_above_thresh: np.ndarray,
                  summed_ensemble_vars: np.ndarray,
                  bodypart_list: list,
                  input_dir: str,
                  likelihood_thresh: float = 0.9) -> None:
    ...
    for kp_idx, kp in enumerate(bodypart_list):
        ...
        for i in range(len(variance_bins) - 1):
            in_bin = (summed_ensemble_vars[:, kp_idx] >= variance_bins[i]) \
                & (summed_ensemble_vars[:, kp_idx] < variance_bins[i + 1])
        ...
        im = axs[row, col].imshow(
            heatmap.T, aspect='auto', origin='lower', extent=[0, len(variance_bins) - 1, 0, 6],
            cmap=cmap, norm=norm)
        ...
        for i in range(heatmap.shape[0]):
            for j in range(heatmap.shape[1]):
                axs[row, col].text(i + 0.5, j + 0.5, f'{heatmap[i, j]:.0f}',
                                   ha='center', va='center', color='black')

        x_ticks = np.arange(len(variance_bins) - 1)
        x_labels = [
            f'({variance_bins[i]:.3f},'
    ...
    output_path = os.path.join(script_dir,
                               f'{os.path.basename(input_dir)}_var_likelihood_heatmap.png')
    ...
    im_total = ax_combined.imshow(
        total_heatmap.T, aspect='auto', origin='lower',
        extent=[0, len(variance_bins) - 1, 0, 6], cmap=cmap, norm=norm_combined)
    ...
    ax_combined.set_title(
        f'Combined Keypoints for {os.path.basename(os.path.dirname(os.path.dirname(input_dir)))}')

    for i in range(total_heatmap.shape[0]):
        for j in range(total_heatmap.shape[1]):
            ax_combined.text(i + 0.5, j + 0.5, f'{total_heatmap[i, j]:.0f}',
                             ha='center', va='center', color='black')
    ...
    combined_output_path = os.path.join(
        script_dir, f'{os.path.basename(input_dir)}_var_likelihood_heatmap_combined.png')
...
def run_inference_on_snippets(model_dirs_list, data_dir, snippets_dir, ground_truth_df):
    ...
    for model_dir in model_dirs_list:
        ...
        model = load_model_from_checkpoint(
            cfg=cfg_new, ckpt_file=ckpt_file, eval=True, data_module=datamodule_new)
        model.to("cuda")
        ...
        for session, frame in tqdm(session_frame_info):
            ...
            pred_handler = PredictionHandler(
                cfg=cfg, data_module=datamodule_new, video_file=video_file)

            # Compute predictions
            preds = trainer.predict(
                model=model, dataloaders=predict_loader, return_predictions=True)
...
def run_eks_on_snippets(snippets_dir: str,
                        model_dirs_list: list,
                        eks_save_dir: str,
                        ground_truth_df: pd.DataFrame,
                        keypoint_ensemble_list: list) -> tuple:
    session_frame_info = process_csv_for_sessions_and_frames(ground_truth_df)
    ...
    for session, frame in session_frame_info:
        ...
        for model_dir in model_dirs_list:
            ...
            keypoint_names = [col[1] for col in df_tmp.columns[::3]]
    ...
    if os.path.exists(final_predictions_file):
        print(
            f'Final predictions file {final_predictions_file} already exists. '
    ...
    for model_dir in tqdm(model_dirs_list):
        ...
        for session in sessions:
            ...
            for frame in frames:
                ...
                df = pd.read_csv(
                    os.path.join(
                        model_dir, 'video_preds_labeled', session, frame.replace('.mp4', '.csv')),
                    header=[0, 1, 2],
                    index_col=0,
                )
                # Extract result from center frame
...
def compute_ens_mean_median(model_dirs_list, eks_save_dir, post_processor_type):
    ...
    if os.path.exists(results_file):
        print(
            f'Predictions file {results_file} already exists. '
            f'Skipping computation for {post_processor_type}.')
        return
...
>>>>>>> 15af09e6f8a0f8253e494077fe1aec15343a317a
    ...
    dataset = get_dataset(
        cfg=model_cfg, data_dir=model_cfg.data.data_dir, imgaug_transform=imgaug_transform)
    data_module = get_data_module(
        cfg=model_cfg, dataset=dataset, video_dir=os.path.join(data_dir, dataset_name, "videos"))
    data_module.setup()
    ...
    for post_processor_type in post_processor_types:
        ...
        if os.path.exists(metrics_file):
            print(
                f'Metrics file {metrics_file} already exists. '
                f'Skipping post-processor {post_processor_type}.')
            continue
...
def pipeline_ood_snippets(
    ...
    """
    Sub-pipeline for Out-of-Distribution (OOD) snippets. Runs inference, EKS,
    prediction collection, ensemble computation, and metric computation.
